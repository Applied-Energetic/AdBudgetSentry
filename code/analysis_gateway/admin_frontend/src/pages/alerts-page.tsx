  ...
  return (
    <div className="space-y-6">
      <Card className="soft-panel">
        <CardHeader className="gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <CardDescription>按账号关键词、发送状态、告警类型和日期范围筛选历史告警。</CardDescription>
          </div>
      ...
      <section className="grid gap-4 md:grid-cols-4">
        <StatCard title="当前结果" value={String(alerts.length)} hint="筛选后的告警结果数量" />
        <StatCard title="已发送" value={String(stats.sent)} hint="成功投递的告警记录" />
        <StatCard title="发送失败" value={String(stats.failed)} hint="需要重点排查的投递失败" />
        <StatCard title="已跳过" value={String(stats.skipped)} hint="由于冷却或规则跳过的记录" />
      </section>

      <Card className="soft-panel">
        <CardHeader>
          <CardTitle>告警历史</CardTitle>
          <CardDescription>默认最多展示 500 条记录，便于在一个页面内回看与导出。</CardDescription>
        </CardHeader>
            ...
            <TableBody>
              {alerts.map((alert) => (
                <TableRow key={alert.id}>
                  ...
                  <TableCell>{alert.alert_kind}</TableCell>
                  <TableCell>{formatDateTime(alert.triggered_at)}</TableCell>
                </TableRow>
              ))}
              {alerts.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={6} className="py-10 text-center whitespace-normal text-muted-foreground">
                    当前筛选条件下没有告警记录。
                  </TableCell>
                </TableRow>
              ) : null}
